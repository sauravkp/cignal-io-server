...
class Room extends EnhancedEventEmitter {
  ...
  createPeer(peerId, transport) {
    ...
    if (this._peers.has(peerId)) {
      logger.debug("Replace transport of the existing Peer");
      const peer = this._peers.get(peerId);
      peer.replaceTransport(transport);
    }
    ...
    peer.on("reset", () => {
      if (peer._disconnectedTimer) {
        logger.warn(
          "this peer has disconnection timeout. clearing it before deleting it;"
        );
        clearTimeout(peer._disconnectedTimer);
      }

      // this._peers.delete(peerId);
      // logger.warn("removing this peer for reconnection!!");
    });
