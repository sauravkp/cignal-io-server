...
class Peer extends EnhancedEventEmitter {
  ...
  _handleTransport() {
    ...
    this._transport.on("close", () => {
      if (this._closed) {
        logger.error("Peer already closed for event close. Nothing to do.");
        return;
      }
      logger.warn(
        "Going to close this user immediately as browser closed or other irrecoverable error happened!"
      );
      this._closed = true;
      this.safeEmit("close");
    });
    ...
    this._transport.on("disconnect", () => {
      ...
      this.data.reconnectionPending = true;
      logger.warn(
        "Going to close this user in  case of a socket reconnection doesnot happen within 2 minutes!"
      );
      this.safeEmit("disconnect");
      this._disconnectedTimer = setTimeout(() => {
        logger.debug(
          "Going to initiate close Peer as it didn't get reconnected in last 120 seconds"
        );
        this.safeEmit("timeOut");
      }, 120000);
    });
    ...
    this._transport.on("reconnected", () => {
      if (this._closed) {
        logger.error(
          "Peer already closed for event reconnected. Nothing to do."
        );
        return;
      }
      logger.error("Peer reconnected!");
      this._connected = true;
      this.data.reconnectionPending = false;
      clearTimeout(this._disconnectedTimer);
      this.safeEmit("reconnect");
    });
    this._transport.on("message", (message) => {
      if (message.method !== "realTimeLoad")
        logger.debug("Transport received message:%s", message);
